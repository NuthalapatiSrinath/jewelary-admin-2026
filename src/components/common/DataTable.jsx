...
const DataTable = ({
  ...
  const [localLimit, setLocalLimit] = useState(50); // Default to 50
  ...
  // Use server pagination or local state
  const limit = isServer ? pagination?.limit || 50 : localLimit;
  ...
  return (
    <div className="flex flex-col h-full w-full max-w-full bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden relative">
      {/* HEADER */}
      <div className="p-5 border-b border-slate-200 flex flex-col sm:flex-row justify-between gap-4 flex-shrink-0 bg-white z-20">
        <div>
        ...
        <div className="flex gap-2 items-center w-full sm:w-auto">
          <div className="relative flex-1 sm:flex-none">
            ...
            <input
              ...
              className="pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-base md:text-sm w-full sm:w-[240px] outline-none focus:border-indigo-500 transition-colors text-slate-900 placeholder:text-slate-400"
            />
          </div>
          {actionButton}
        </div>
      ...
      <div className="flex-1 relative w-full overflow-hidden flex flex-col bg-white">
        <AnimatePresence>
          {loading && (
            ...
            >
              <div className="bg-white p-4 rounded-lg shadow-lg border border-slate-200 flex items-center gap-3">
      ...
      {/* FOOTER */}
      <div className="px-6 py-5 md:px-4 md:py-3 border-t border-slate-200 bg-white z-20 flex flex-col sm:flex-row gap-5 items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-3 text-base md:text-sm text-slate-500 font-medium">
          <span>Show</span>
          <select
            value={limit}
            onChange={(e) => changeLimit(Number(e.target.value))}
            className="bg-slate-50 border border-slate-200 rounded px-3 py-1.5 md:py-1 outline-none focus:border-indigo-500 cursor-pointer text-base md:text-sm text-slate-700"
          >
            {[10, 20, 50, 100].map((n) => (
